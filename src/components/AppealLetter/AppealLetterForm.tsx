...
import { medications, insurances } from "../suggestions"; // Import suggestions
import axios from "axios";

const AppealLetterForm: React.FC = () => {
  const [medication, setMedication] = useState<string>("");
  const [insurance, setInsurance] = useState<string>("");
  const [chartNotesFile, setChartNotesFile] = useState<File | null>(null);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!chartNotesFile || !denialLetterFile) {
      setResponse("Please upload both the progress notes and the denial letter.");
      return;
    }
    ...
    formData.append("mode", "appeal");
    formData.append("medication_name", medication);
    formData.append("insurance_name", insurance);
    formData.append("chart_notes", chartNotesFile);
    formData.append("denial_letter", denialLetterFile);
    ...
    try {
      ...
      if (res.data.message) {
        ...
        setResponse("Failed to generate the appeal letter.");
      }
  ...
  const handleChartNotesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      setChartNotesFile(e.target.files[0]);
    }
  };

  const handleDenialLetterChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) {
      setDenialLetterFile(e.target.files[0]);
    }
  };
    ...
    <section className="py-16 md:py-24">
      <div className="max-w-[1000px] mx-auto bg-white shadow-xl rounded-2xl p-8 md:p-12">
        ...
        <form className="space-y-6 md:space-y-8" onSubmit={handleSubmit}>
          ...
          <div>
            <label className="block text-base md:text-lg text-gray-600 mb-4" htmlFor="chart-notes-upload">
            ...
            <input
              ...
              accept=".pdf"
              onChange={handleChartNotesChange}
          ...
          <div>
            <label className="block text-base md:text-lg text-gray-600 mb-4" htmlFor="denial-letter-upload">
            ...
            <input
              ...
              accept=".pdf"
              onChange={handleDenialLetterChange}
              className="w-full px-4 py-3 md:px-6 md:py-4 text-sm md:text-base rounded-full bg-gray-100 border border-gray-300 focus:outline-none focus:border-blue-600"
              required
            />
          </div>
...
export default AppealLetterForm;
